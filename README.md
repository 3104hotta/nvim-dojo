...
[`dojo/`](dojo/) に練習用のヘルパーがある。プラグインなしで動く Lua 単体ファイル。

| ファイル | 用途 |
|----------|------|
| [`dojo/hjkl-warn.lua`](dojo/hjkl-warn.lua) | 同方向の `hjkl` を3回以上連続で押すと警告を出す |

使い方:

```vim
:luafile dojo/hjkl-warn.lua    " このセッションだけ有効化
```

詳細・しきい値変更・解除方法は各ファイル冒頭のコメントを参照。
