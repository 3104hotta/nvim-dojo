...
// Summary: three repeat groups
// Group 1 (dot): replace Box::new(Error::from(...)) with Error::from(...).into()  (5 places)
// Group 2 (macro): convert .map_err(|e| log::error!(...))? pattern  (10 places)
// Group 3 (N prefix): add /// # Example to doc-comment blocks  (5 blocks) + indent range
...
// Group 1: Box::new(Error::from(...)) → Error::from(...).into()
pub fn wrap_1(e: std::io::Error) -> Box<AppError> { Box::new(AppError::from(e)) }
pub fn wrap_2(e: std::io::Error) -> Box<AppError> { Box::new(AppError::from(e)) }
pub fn wrap_3(e: std::io::Error) -> Box<AppError> { Box::new(AppError::from(e)) }
pub fn wrap_4(e: std::io::Error) -> Box<AppError> { Box::new(AppError::from(e)) }
pub fn wrap_5(e: std::io::Error) -> Box<AppError> { Box::new(AppError::from(e)) }

impl From<std::io::Error> for AppError {
    fn from(e: std::io::Error) -> Self { AppError::Io(e) }
}

// Group 2: .map_err with log (macro target)
pub fn step_a(data: &[u8]) -> Result<Vec<u8>, AppError> {
    std::fs::read("/tmp/a").map_err(|e| log::error!("{}", e)).map_err(AppError::Io)?;
    Ok(data.to_vec())
}
pub fn step_b(data: &[u8]) -> Result<Vec<u8>, AppError> {
    std::fs::read("/tmp/b").map_err(|e| log::error!("{}", e)).map_err(AppError::Io)?;
    Ok(data.to_vec())
}
pub fn step_c(data: &[u8]) -> Result<Vec<u8>, AppError> {
    std::fs::read("/tmp/c").map_err(|e| log::error!("{}", e)).map_err(AppError::Io)?;
    Ok(data.to_vec())
}
